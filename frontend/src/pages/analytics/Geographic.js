...
const Geographic = () => {
  ...
  useEffect(() => {
    if (locationsQuery.isSuccess && locations.length > 0) {
      ...
      setTimeout(() => {
        // Use the custom classname for leaflet map container to select it
        const mapContainer = document.querySelector(".leaflet-container");
        if (!mapContainer) return;

        // Leaflet attaches a reference to the map instance for react-leaflet setup
        const map = mapContainer._leaflet_map;
        if (!map) return;

        // Clear existing heat layers
        map.eachLayer(layer => {
          // This check assumes the heat layer has an 'options' property with a unique key like 'heat'
          // which is often true for custom layers, but a more robust check might be needed
          // if you have other custom layers. For now, we'll keep the existing check.
          if (layer.options && layer.options.heat) {
            map.removeLayer(layer);
          }
        });

        // The heat layer expects an array of [latitude, longitude, intensity]
  ...
  const processBarangayTypeData = (data) => {
    ...
    // Get all unique assistance types to build the keys for the chart
    const allTypes = [...new Set(safeData.map(item => item.type_of_assistance).filter(t => t))];

    const resultData = barangays.slice(0, 6).map(barangay => {
    ...
    // Attach the list of types to the function result for use in the chart
    resultData.types = allTypes;
    return resultData;
  };
  ...
  // Top barangays for the bar chart
  const topBarangaysData = topBarangaysQuery.isSuccess ? (topBarangays || []).slice(0, 8).map(item => ({
  ...
  const assistanceTypes = chartData.types || [];
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-blue-100 relative">
      ...
      <div className="relative z-10 p-6 space-y-6">
        ...
        {isLoading ? (
          ...
          {/* Analytics Grid */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Top Barangays Chart (Bar Chart) */}
            {topBarangaysQuery.isLoading ? <LoadingChartSkeleton title="Loading Top Barangays..." /> : (
                <div className="bg-white bg-opacity-80 backdrop-blur-xl rounded-3xl shadow-xl p-6 border border-blue-200">
                  ...
                  {topBarangaysData.length > 0 ? (
                      ...
                      <div className="h-[280px] flex items-center justify-center text-gray-500">
                          No top barangay data available.
                      </div>
                  )}
                </div>
            )}

            {/* Approval Rates by Location (Pie Chart) - Original Second Item */}
            {approvalRatesQuery.isLoading ? <LoadingChartSkeleton title="Loading Approval Rates..." /> : (
              <div className="bg-white bg-opacity-80 backdrop-blur-xl rounded-3xl shadow-xl p-6 border border-green-200">
                <div className="flex items-center gap-3 mb-6">
          ...
          {/* Barangay by Type Chart (Stacked Bar Chart) */}
          {barangayByTypeQuery.isLoading ? <LoadingChartSkeleton height={350} title="Loading Assistance Types Distribution..." /> : (
              <div className="bg-white bg-opacity-80 backdrop-blur-xl rounded-3xl shadow-xl p-6 border border-purple-200">
                  <div className="flex items-center gap-3 mb-6">
                      ...
                      <h3 className="text-xl font-bold text-gray-900">
                          Assistance Type Distribution by Barangay (Top 6)
                      </h3>
                  </div>
                  {chartData.length > 0 ? (
                      <>
                          <ResponsiveContainer width="100%" height={350}>
                              ...
                              >
                                  <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                                  <XAxis dataKey="barangay" stroke="#6b7280" tick={{ fontSize: 12 }} />
                                  <YAxis stroke="#6b7280" />
                                  <RechartsTooltip content={<CustomTooltip />} />
                                  <Legend />
                      ...
                      <div className="h-[350px] flex items-center justify-center text-gray-500">
                          No assistance type distribution data available.
                      </div>
                  )}
              </div>
          )}
