...
const Card = ({ title, icon: Icon, children, gradient = "from-indigo-600 to-blue-700" }) => (
    <div className="bg-white rounded-3xl shadow-xl p-6 sm:p-8 border border-gray-100 transition-all duration-300 hover:shadow-2xl h-full w-full">
        <div className="flex items-center gap-4 mb-6">
            <div className={`w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br ${gradient} rounded-lg flex items-center justify-center shadow-lg flex-shrink-0`}>
...
const StatCard = ({ icon: Icon, title, value, gradient, colorHex }) => (
    <div className={`bg-white rounded-2xl shadow-lg p-5 sm:p-6 border border-gray-100 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 w-full`}>
        <div className="flex justify-between items-start mb-4">
            <div className="flex-1 min-w-[60%]">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">{title}</p>
                <p className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 break-words">{value}</p>
            </div>
            <div className={`w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br ${gradient} rounded-xl flex items-center justify-center shadow-lg flex-shrink-0 ml-4`}>
    ...
    const fetchDashboardData = useCallback(async () => {
        ...
        try {
            const [
                ...
                api.get("/analytics/breakdown/type/"), 
                api.get("/analytics/funnel/status/"),
            ]);
            ...
            // Set Analytics Data from your API
            setTypeBreakdown(breakdownRes.data || []);
        ...
        if (active && payload && payload.length) {
            return (
                <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-xl text-sm">
                    ...
                    <p className="text-blue-600">Total: <span className="font-bold">{payload[0].value.toLocaleString()}</span></p>
                </div>
            );
        }
        return null;
    ...
    return (
        <div className="p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-6 bg-gray-50 min-h-screen">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                ...
                <Card title="Application Type Breakdown" icon={PieIcon} gradient="from-teal-600 to-cyan-700">
                    {loading ? (
                        <ChartSkeleton />
                    ) : (
                ...
                <Card title="Application Status Funnel" icon={BarIcon} gradient="from-red-600 to-pink-700">
                    {loading ? (
                        <ChartSkeleton />
                    ) : (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 sm:gap-6">
                ...
                <div className="lg:col-span-2">
                    <Card title="Staff Activity Leaderboard" icon={Activity} gradient="from-purple-600 to-indigo-700">
                        {loading ? (
                                    ...
                                    {staffActivity.slice(0, 5).map((s, i) => (
                                        ...
                                        >
                                            <div className="flex items-center gap-4">
                                                ...
                                                <span className="font-semibold text-gray-800 truncate">{s.staff__username}</span>
                                            </div>
                ...
                <div className="lg:col-span-3">
                    <Card title="Recently Submitted Applications" icon={FileText} gradient="from-green-600 to-teal-700">
                        {loading ? (
                            <ListSkeleton items={5} />
                        ) : (
                                ...
                                <table className="w-full text-sm border-collapse table-auto">
                                    <thead>
                                        <tr className="bg-gray-100 border-b border-gray-200">
                                            <th className="p-3 sm:p-4 text-left font-bold text-gray-700 min-w-[130px]">Applicant Name</th>
                                            <th className="p-3 sm:p-4 text-center font-bold text-gray-700 min-w-[90px]">Barangay</th>
                                            <th className="p-3 sm:p-4 text-center font-bold text-gray-700 min-w-[80px]">Type</th>
                                            <th className="p-3 sm:p-4 text-right font-bold text-gray-700">Timestamp</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {(recentApplicants && recentApplicants.slice(0, 5).map((a, idx) => (
                                            ...
                                            >
                                                ...
                                                <td className="p-3 sm:p-4 text-center text-gray-600 truncate max-w-[90px]">{a.background_info?.barangay}</td>
                                                <td className="p-3 sm:p-4 text-center">
                                                    <span className={`px-2 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap ${getTypeStyles(a.type_of_assistance)}`}>
                                                        {a.type_of_assistance}
                                                    </span>
                                                </td>
                                                <td className="p-3 sm:p-4 text-right text-gray-500 text-xs">{formatDate(a.date_filled)}</td>
                                            </tr>
                                        ))) || null}
                                    </tbody>
                                </table>
