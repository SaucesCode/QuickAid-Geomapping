...
const PrintPage = () => {
  ...
  // ✅ Detect if staff is logged in (has token in localStorage)
  const isStaffLoggedIn = Boolean(localStorage.getItem("accessToken"));
  ...
  useEffect(() => {
    if (!applicant) {
      if (isStaffLoggedIn) {
        ...
        navigate("/"); // go to home if no applicant + no login
      }
    }
