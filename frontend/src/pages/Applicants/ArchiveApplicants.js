...
// --- API Fetcher for useQuery ---
const fetchArchivedApplicantsList = async () => {
    // Add the delay here to make the skeleton noticeable, as in the original logic
...
const ArchiveApplicants = () => {
    // Original State Variables (kept, but managed by React Query where applicable)
    // The state variable 'archivedApplicants' is replaced by React Query's data, but
    // we use a Memo to hold the filtered data which is the logical replacement for the old state.
    const [archivedApplicants, setArchivedApplicants] = useState([]); // This variable is now technically unused but kept as requested
    // The 'loading' state is now derived from React Query
    ...
    // 2. Replace manual fetchArchivedApplicants with useQuery
    const { data: applicantsData = [], isLoading, isFetching, isError } = useQuery({
        queryKey: ["archivedApplicants"],
        queryFn: fetchArchivedApplicantsList,
    });
    
    // We use a derived value to represent the main list of applicants
    // If the original 'archivedApplicants' variable *must* be set, you could use a useEffect:
    // useEffect(() => { setArchivedApplicants(applicantsData); }, [applicantsData]);
    // However, to keep the logic clean, we use the `applicantsData` variable directly.
    const allArchivedApplicants = applicantsData;
    const loading = isLoading || isFetching; // Combined loading state

    // 3. Mutation Hook for Restore
    const restoreApplicantMutation = useMutation({
        mutationFn: (applicantId) => api.post(`/restore-applicant/${applicantId}/`),
        onSuccess: () => {
            // Invalidate and re-fetch the list automatically after a successful restore
            queryClient.invalidateQueries({ queryKey: ["archivedApplicants"] });
            // Optional: Invalidate the active applicants list if it's on a different page/component
    ...
    const handleRestore = () => {
        ...
        restoreApplicantMutation.mutate(restoreModal.applicantId);
    };
    ...
    const filteredApplicants = useMemo(() => {
        return allArchivedApplicants.filter(a => {
    ...
    }, [allArchivedApplicants, searchTerm]);
    ...
    useEffect(() => {
        // Removed fetchArchivedApplicants(), React Query handles initial fetch
                    ...
                    <div className="bg-white bg-opacity-90 backdrop-blur-xl rounded-2xl sm:rounded-3xl shadow-lg border border-gray-200 p-6 sm:p-8">
                        ...
                        <div className="flex items-center gap-3 mt-4 sm:mt-6">
                            <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-50 text-blue-700 rounded-full border border-blue-200 text-sm font-semibold">
                                ...
                                <span>{allArchivedApplicants.length} Archived Records</span>
                            </div>
                        </div>
                    </div>
                </div>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button
                                                        ...
                                                        // Disable button while a restore is in progress
                                                        disabled={restoreApplicantMutation.isPending} 
                                                    >
                                        ...
                                        <td colSpan="6" className="px-6 py-24 text-center bg-gray-50">
                                            <div className="flex flex-col items-center">
                                                ...
                                                <h3 className="text-2xl font-bold text-blue-800 mb-2">
                                                    {isError ? "Error fetching applicants" : "No archived applicants found"}
                                                </h3>
                        ...
                        <div className="bg-white rounded-xl shadow-2xl w-full max-w-md border border-blue-200 overflow-hidden animate-in fade-in zoom-in duration-200">
                            ...
                            <div className="flex justify-end gap-3 p-5 border-t border-blue-100 bg-blue-50">
                                ...
                                <button
                                    ...
                                    className="inline-flex items-center gap-2 px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all shadow-md font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                                    disabled={restoreApplicantMutation.isPending}
                                >
                                    {restoreApplicantMutation.isPending ? (
                                        <>
