...
const initialFormData = {
  ...
  // Step 2
  street_address: "", province: "Quezon", city_municipality: "", city_municipalityCode: "",
  barangay: "", barangay_name: "", birthday: "", sex: "", civil_status: "", occupation: "", monthly_income: "",
  // Step 3
  valid_id_presented: "", other_valid_id: "", applicant_type: "Self", type_of_assistance: "",
  created_at: new Date().toISOString(),
  // Representative
  rep_first_name: "", rep_middle_initial: "", rep_last_name: "", rep_suffix: "", rep_address: "",
  rep_birthday: "", rep_gender: "", rep_civil_status: "", rep_occupation: "", rep_monthly_income: "",
  rep_relationship: "", use_same_address: false,
};
...
const REPRESENTATIVE_FIELDS = [
  "rep_first_name", "rep_middle_initial", "rep_last_name", "rep_suffix", "rep_address",
  "rep_relationship", "rep_occupation", "rep_province", "rep_city_municipality", "rep_barangay_name",
];
                ...
                <div className="flex flex-col items-center">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${
                      step === stepNum ? "bg-blue-500 text-white shadow-lg shadow-blue-500/50 scale-105" :
                      step > stepNum ? "bg-blue-100 text-blue-600" : "bg-gray-100 text-gray-400"
                    }`}
                  >
                    {step > stepNum ? <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg> : stepNum}
                  </div>
                </div>
                {stepNum < 4 && (
                  <div className={`w-16 h-1 rounded-full transition-all duration-300 mx-2 ${step > stepNum ? "bg-blue-400" : "bg-gray-200"}`} />
                )}
              </React.Fragment>
            ))}
          </div>
      ...
      {cancelModal.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-11/12 max-w-xs p-5"> 
            <h2 className="text-lg font-bold text-gray-800 text-center mb-2">Cancel Application?</h2> 
            <p className="text-gray-600 text-center mb-4 text-xs"> 
              All entered information will be <span className="font-bold text-red-600">permanently lost</span>.
            </p>
            <div className="flex gap-2"> 
              <button onClick={closeCancelModal} className="flex-1 px-3 py-1.5 text-sm rounded-lg bg-blue-500 hover:bg-blue-600 text-white font-semibold"> 
                No, Keep Editing
              </button>
              <button onClick={confirmCancel} className="flex-1 px-3 py-1.5 text-sm rounded-lg bg-white hover:bg-gray-50 text-gray-700 font-semibold border border-gray-200"> 
                Yes, Cancel
              </button>
            </div>
          </div>
